...
class TestWord:
    ...
    excluded_inverse_normalizer_en = (
        (
            InverseNormalizer(
                lang='en',
                cache_dir=CACHE_DIR,
                overwrite_cache=False,
                language_excluded_classes={'en': {'word': True}},
    ...
    @parameterized.expand(parse_test_case_file('en/data_inverse_text_normalization/test_cases_exclude_word.txt'))
    @pytest.mark.skipif(
        not PYNINI_AVAILABLE,
        reason="`pynini` not installed, please install via nemo_text_processing/pynini_install.sh",
    )
    ...
    def test_excluded_denorm(self, test_input, expected):
        pred = self.excluded_inverse_normalizer_en.inverse_normalize(test_input, verbose=False)
        assert pred == expected
